...
- Extensions for collection properties no longer attempt to initialize the collection if the type has no nominated initializer via `AutoInitializeWith` and the type has no accessible default constructor.
...
- Immutable collection contracts are no longer treated as mutable collections when generating member-appending extensions.
...
- `FLTFY03`, `FLTFY05`, and `FLTFY11` now only report when `Descriptor`, `Ignore`, or `Hide` are used on types that are `sealed` and not annotated with `Fluentify`.

# [2.2.2] - 2026-05-15

## Fixed

- Properties with explicit interface implementations are now excluded.
- Properties with type ICollection or IList are now correctly identified as collection types, and have builder-based extensions generated accordingly.
- Nullability is now determined by language version instead of framework version.

# [2.2.1] - 2026-05-14

## Fixed

- Mutable derived properties no longer produce duplicate fluent extension methods.
